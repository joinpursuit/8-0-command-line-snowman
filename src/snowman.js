...
//Reset values using destructuring for Function definitions
function resetValues (global) {
  switch(global) {
    case Array.isArray(global) : 
      return global = [];
    case typeof global === "string" : 
      return global = "";
    case typeof global === "number" :
      return global = 0;
    case typeof global === "boolean" :
      return global = false;
    case global === word :
      global = "";
      return global = getRandomWord();
    case global === guesses :
      return global = process.argv[2] || word.length;
  };
}; 
...
function run() {
  ...
  console.log(chalk.purple("----------Command Line Snowman Game----------\n\n"));

  if (!scoreWins) {
    console.log(chalk.green("Game Play: Guess the word below. First, try guessing by letter. However, each time you guess a letter incorrectly, you lose a try to guess. At a certain point in the game, there's an option to guess the whole word. At the end of each game, you may continue playing new games. Once you're done, you'll see how many times you won and lost throughout the series. \n\n"));
  };

  console.log(chalk.purple("----------The Snowman Game----------\n\n Remaining Incorrect Letter Guesses: "  + guesses + "\n " + hiddenLetters(word) + "\n\n")); 
  ...
  while (word !== hiddenWord && guesses) {
    ...
    dashCount = hiddenArray.filter(letter => letter === "_").length;
      while (dashCount <= 2 && wordGuesses) {
        let makeGuess = readline.question("Would you like to guess the word? You have " + wordGuesses + " tries. Y/N?");
