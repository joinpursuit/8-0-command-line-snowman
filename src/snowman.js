...
function run() {
  ...
  while (guessCounts > 0) {
    ...
    if (!alphabet(userInput)) {
      console.log("Invalid input. Only letters. One by one pretty pls.");
    }
    ...
    if (word.toLowerCase() === dashes.join("").toLowerCase()) {
      //then congrats
      console.log(`The word was ${word}! Wonderful job honeypie Great work (: `);
      return;
    }
  }
  ...
  if (guessCounts === 0) {
    ...
    console.log("Better luck next time! Don't be a quitter... Lets Try again.");
  }
